...
| F233 | Phase 6 runtime: forces.rs double-rotates input forces by yaw (input.rs already camera-relative) AND treats degrees as radians | Critical | RESOLVED | Removed yaw rotation from accumulate_forces; input.rs already rotates by camera.yaw. |
| F234 | Phase 6 runtime: character spawns at Startup before terrain loads async, falls through world | Critical | RESOLVED | Added teleport_to_terrain_system in GamePlugin (Update) — detects terrain load, teleports player+camera to surface, resets velocity. |
| F235 | Phase 6 runtime: sync_camera_to_player (PostUpdate) overrides camera_terrain_init_system (Update) every frame | High | RESOLVED | sync_camera_to_player now recomputes view matrix after position change; F238 fix adds HEIGHT_SCALE conversion. |
| F236 | Phase 6 runtime: black screen — no terrain, world, or water visible at runtime despite 337 tests passing | Critical | RESOLVED | Compound of F233-F235+F238. All sub-issues fixed. |
...
| F244 | Water still appears as blank or near-invisible surface after porting original `render.cpp` water motion ideas. Tried: animated water-plane bob, world-space Perlin coordinate sampling, and stricter underwater ripple gating. All failed to materially fix the symptom. Reflections remained visible but water still read as clear/blank. | Critical | OPEN | Investigation showed the problem is broader than missing Perlin or missing water-plane animation. See `docs/worksheets/plans/2026-03-09-water-regression-investigation-log.md`. |
...
| F251 | The second explicit Render Tuning Workbench implementation (`93f4b37` + working tree, 2026-04-21) remains not user-acceptable after live launch. Evidence: user reported that after `Resume Scene` there is no path back to the workbench, rain has no visible effect, terrain/world culling observability is unclear, the workbench lacks an ASCIIID-style spin toggle, right-side numeric readouts get hidden, glyph matching lacks user-selectable/custom glyph sets, and the Shadows button appears to do nothing. | Critical | MONITORING | Root cause: the workbench fixed entry/click/scroll mechanics but still lacked ASCIIID-derived viewport controls, control-effect proof/readouts, custom glyph selection, visible pass deltas, and round-trip navigation. Implementation pass `a84016f` added return navigation, spin/orbit, pass proof, custom glyph candidates, palette diagnostics, and stable readouts; awaiting live user approval. See `docs/worksheets/research/2026-04-21-asciiid-font-palette-material-audit.md`. |
| F252 | Render Tuning Workbench follow-up (`a84016f` plus local palette-preview experiment, 2026-04-21) still does not provide a user-friendly glyph/matching design. Evidence: user reported that "custom glyphs / glyph matching is still too confusing" and requested a Figma-style preset design that is more customizable, real default palettes instead of monochrome remaps, and adjustable elevation/material lanes. | Critical | MONITORING | Root cause: the implementation exposed low-level candidate glyphs and monochrome/simple color remaps without a higher-level preset/theme model or material-ramp/elevation lane controls. Current working tree adds named glyph/matching presets, multi-color Vim/editor-inspired palette previews, and material/elevation lane preview controls; awaiting live user approval. |
...
*Failure log last updated: 2026-04-21 (F250 added after rejected first render UI implementation on commit `59863c6`)*
...
- Evidence:
  - `docs/worksheets/plans/2026-03-10-architecture-alignment-handoff.md`
...
## 2026-04-21 Source Path Addendum

- Date/HEAD: 2026-04-21, `6c9937e` on `planning-audit-normalization`
- What failed: canon and worksheet docs still referenced the old local original-game root
  `/Users/r/Downloads/asciicker-Y9-2/`, which does not exist in this checkout.
- Evidence:
  - active docs still pointed at `/Users/r/Downloads/asciicker-Y9-2/...`
  - the available local original source is `/Users/rikihernandez/Downloads/Aciicker-Y9-2/`
  - the editor source required for durable references is now vendored at
    `reference/original-game/asciiid.cpp`
- Root cause: documentation was carried forward from an older machine-specific
  environment and never normalized to the current workspace.
...
- Status: MONITORING; return navigation, spin/orbit, pass proof, weather/culling
  readouts, custom glyph candidates, and unclipped diagnostics implemented in
  `a84016f`, pending live user approval.

## 2026-04-21 Render Workbench Preset And Palette Design Regression

- Date/HEAD: 2026-04-21, `a84016f` on `planning-audit-normalization`
- What failed: the follow-up workbench direction still makes glyph matching too
  low-level and confusing for user approval.
- Evidence:
  - user report: "custom glyphs / glyph matching is still too confusing"
  - user requirement: "i need a user friendly design with presets in the
    similar way that figma html page provides preset, but more customizable"
  - user requirement: "expand the defauult pallettes to actually be palletes,
    not monochrome, see vim themes for inspirations"
  - user requirement: "also needs adjustable lanes for elevations and stuff
    like that too"
- Root cause: the current design exposes raw candidate glyph grids and simple
  palette remaps before providing task-oriented presets, theme palettes, and
  material/elevation lanes that explain the final render path.
- Status: MONITORING; current working tree implements named glyph/matching
  presets, multi-color palette themes, and material/elevation lane preview
  controls, pending live user approval.
