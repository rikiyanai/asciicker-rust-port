...
|---|---:|---|---|
| Original C++ engine architecture | 68% | 6-stage CPU pipeline, BSP visibility query, terrain + mesh raster, shadow stage, physics/world/game state all exist | sprite XP blit still placeholder, world reflections still incomplete, and this worksheet predates the canonical render-workbench target |
...
Still not aligned:
- `engine-port/src/render/sprite_blit.rs` is still the placeholder sprite path, not full XP sprite compositing.
- `engine-port/src/render/water.rs` still has outstanding work and should remain last per current direction.
- This worksheet used parity language as the product target; canon now treats the render workbench as the target and parity traces as reference evidence only.
...
- Deferred:
  - sprite XP blit completion (reason: intentionally left until after render/core/workbench shell work)
  - water rendering cleanup (reason: explicitly deprioritized until upstream gaps are closed)
...
- Open Risks:
  ...
  - canonical render workbench target is not represented in this older worksheet
