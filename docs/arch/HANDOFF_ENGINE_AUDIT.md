...
Map the ENTIRE Asciicker C++ game engine — every function, every global, every data flow — into a single comprehensive architecture document. The output lives at `docs/ENGINE_ARCHITECTURE.md`.
...
**NEW RULE: Every Wave 1 agent MUST write its output to `docs/arch/<filename>.md` using the Write tool.** The conversation context is only used for coordination, not for storing analysis results. If context dies, the files on disk survive.
...
|------|-------|----------------|--------------|
| `game.cpp` | 11614 | Agent A: lines 1-4000 (init, player, movement). Agent B: lines 4001-8000 (combat, AI, items). Agent C: lines 8001-11614 (UI, rendering calls, main loop) | `docs/arch/game_cpp_part1.md`, `docs/arch/game_cpp_part2.md`, `docs/arch/game_cpp_part3.md` |
| `asciiid.cpp` | 11584 | Agent A: lines 1-4000 (init, terrain editing). Agent B: lines 4001-8000 (mesh editing, UI panels). Agent C: lines 8001-11584 (MCP commands, render loop, IO) | `docs/arch/asciiid_cpp_part1.md`, `docs/arch/asciiid_cpp_part2.md`, `docs/arch/asciiid_cpp_part3.md` |
| `world.cpp` | 5832 | Agent A: lines 1-3000 (BSP, loading, mesh library). Agent B: lines 3001-5832 (instances, queries, serialization) | `docs/arch/world_cpp_part1.md`, `docs/arch/world_cpp_part2.md` |
| `stb_vorbis.cpp` | 5684 | 1 agent only. VENDORED code (Sean Barrett). Document only: public API functions, how asciicker calls it, any local modifications. Do NOT line-by-line document vendored internals. | `docs/arch/stb_vorbis_cpp.md` |

### TIER 2: LARGE files (2000-5000 lines) — 1-2 agents per file

| File | Lines | Notes | Output File |
|------|-------|-------|-------------|
| `render.cpp` | 4579 | THE CORE RENDERER. 2 agents: A=rasterizer+shaders (first half), B=compositing+output (second half) | `docs/arch/render_cpp_part1.md`, `docs/arch/render_cpp_part2.md` |
| `game_app.cpp` | 3765 | 2 agents: A=app lifecycle+window (first half), B=event loop+platform (second half) | `docs/arch/game_app_cpp_part1.md`, `docs/arch/game_app_cpp_part2.md` |
| `terrain.cpp` | 3310 | 2 agents: A=quadtree+heightmap (first half), B=raycasting+editing (second half) | `docs/arch/terrain_cpp_part1.md`, `docs/arch/terrain_cpp_part2.md` |
| `mainmenu.cpp` | 2844 | 1 agent. Menu system, character creation, UI flow | `docs/arch/mainmenu_cpp.md` |
| `x11.cpp` | 2373 | 1 agent. X11 platform backend. Document public interface + key event handling | `docs/arch/x11_cpp.md` |
| `physics.cpp` | 2351 | 1 agent. Collision detection, gravity, movement physics | `docs/arch/physics_cpp.md` |
| `gamepad.cpp` | 2318 | 1 agent. Controller input mapping, dead zones, button state | `docs/arch/gamepad_cpp.md` |
| `sprite.cpp` | 2023 | 1 agent. XP sprite loading, palette quantization, atlas management | `docs/arch/sprite_cpp.md` |
| `mswin.cpp` | 1942 | 1 agent. Windows platform backend | `docs/arch/mswin_cpp.md` |

### TIER 3: MEDIUM files (500-2000 lines) — 1 agent per batch, write to batch file

| Batch | Files | Output File |
|-------|-------|-------------|
| Platform | `sdl.cpp` (1446) | `docs/arch/batch_sdl.md` |
| Audio | `audio.cpp` (1373) + `audio.h` (123) | `docs/arch/batch_audio.md` |
| Network | `game_svr.cpp` (1236) + `network.cpp` (979) + `network.h` (209) | `docs/arch/batch_network.md` |
| Web | `game_web.cpp` (973) + `game_web.html` | `docs/arch/batch_web.md` |
| Terminal | `term.cpp` (919) + `terminal.cpp` (440) + `term.h` (74) | `docs/arch/batch_terminal.md` |
| Undo | `urdo.cpp` (897) + `urdo.h` (81) | `docs/arch/batch_undo.md` |
| Inventory | `inventory.cpp` (759) + `inventory.h` (233) | `docs/arch/batch_inventory.md` |
| API | `game_api.cpp` (685) + `game_api.h` (104) | `docs/arch/batch_api.md` |
| GL | `imgui_impl_opengl3.cpp` (588) + `gl45_emu.cpp` (365) + `gl45_emu.h` (74) | `docs/arch/batch_gl.md` |
| Color | `rgba8.cpp` (578) + `rgba8.h` (32) | `docs/arch/batch_color.md` |

### TIER 4: SMALL files (<500 lines) — 1 agent per batch

| Batch | Files | Output File |
|-------|-------|-------------|
| Small-A | `weather.cpp` (449), `weather.h` (98), `font1.cpp` (388), `font1.h` (~), `sprite_validate.cpp` (340), `enemygen.cpp` (292), `enemygen.h` (70), `screen.cpp` (93) | `docs/arch/batch_small_a.md` |
| Small-B | `texheap.cpp` (207), `texheap.h` (116), `water.cpp` (70), `world_patch.cpp` (59), `input.cpp` (72), `fast_rand.h` (16), `stdafx.h` (15) | `docs/arch/batch_small_b.md` |

### TIER 5: Header-only architecture files — 1 agent

| Files | Output File |
|-------|-------------|
| `game.h` (567), `world.h` (219), `render.h` (137), `terrain.h` (218), `sprite.h` (111), `sprite_constants.h` (93), `matrix.h` (515), `platform.h` (488), `lexer.h` (1224), `physics.h` (122), `upng.h` (125) | `docs/arch/batch_headers.md` |

---

## Total Output Files: 34 files in `docs/arch/`

Orchestrator tracks progress by checking which files exist:
```bash
ls -la docs/arch/*.md | wc -l   # should reach 34 after Wave 1
```
...
```bash
ls docs/arch/*.md | wc -l              # target: 34
wc -l docs/arch/*.md                    # each file should be substantial
```
...
1. Reads all 34 files from `docs/arch/` using the Read tool
2. Merges them into a single `docs/ENGINE_ARCHITECTURE.md` using Write tool
...
**Assembly agent output file:** `docs/ENGINE_ARCHITECTURE.md`

### CHECKPOINT 2: Verify assembly

```bash
wc -l docs/ENGINE_ARCHITECTURE.md      # target: >3000 lines
grep -c "^### " docs/ENGINE_ARCHITECTURE.md  # count function entries
```

### Wave 3: Triple verification (3 agents in parallel)

Launch 3 independent verifier agents. Each reads `docs/ENGINE_ARCHITECTURE.md` + source files.

**Verifier A — Completeness Check** (output: `docs/arch/verify_completeness.md`):
...
**Verifier B — Accuracy Spot-Check** (output: `docs/arch/verify_accuracy.md`):
...
**Verifier C — Cross-Reference Integrity** (output: `docs/arch/verify_crossrefs.md`):
...
Final `docs/ENGINE_ARCHITECTURE.md` must have:
...
4. **Assembly agent reads from disk** — it reads `docs/arch/*.md`, not conversation history
5. **Verifiers read from disk** — they read `docs/ENGINE_ARCHITECTURE.md`, not conversation
...
1. `ls docs/arch/*.md | wc -l` — see how many files were written
2. `wc -l docs/arch/*.md` — see which are substantial vs stubs
