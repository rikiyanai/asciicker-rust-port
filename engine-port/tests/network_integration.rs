...
use bevy_replicon_renet2::netcode::{
    ClientAuthentication, MemorySocketClient, NetcodeClientTransport, NetcodeServerTransport,
...
fn create_server_transport(
    num_clients: usize,
) -> (NetcodeServerTransport, Vec<MemorySocketClient>) {
...
fn test_server_client_join_exchange() {
    ...
    assert!(
        connected,
        "Client should connect to server via memory transport"
    );
